...
import jittor as jt
from jittor.transform import ImageNormalize, ToTensor, Resize, CenterCrop, RandomHorizontalFlip
...
def normalize(img, mask=None):
    img = ToTensor()(img)
    img = ImageNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(img)
    if mask is not None:
...
def obtain_cutmix_box(img_size, p=0.5, size_min=0.02, size_max=0.4, ratio_1=0.3, ratio_2=1/0.3):
    mask = jt.zeros((img_size, img_size))
    if random.random() > p:
        return mask
